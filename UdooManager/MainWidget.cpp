...
void MainWidget::changeTrackCount(int newCount) {
  ...
  if (newCount < oldCount)  // Suppression
  {
    /*		int ret = QMessageBox::warning(this, tr("Éditeur de morceaux"),
                                                               tr("Le nombre de
    pistes diminue.\n" "Êtes-vous sûr ?"), QMessageBox::Yes | QMessageBox::No,
                                                               QMessageBox::Yes);
...
void MainWidget::save() {
  savemanager->save(QFileDialog::getSaveFileName(this, "Sauvegarder", QString(),
                                                 "Morceau (*.song)"));
}

void MainWidget::load() {
  QString file = QFileDialog::getOpenFileName(this, "Charger", QString(),
                                              "Morceau (*.song)");
  if (!file.isEmpty())
...
void MainWidget::addChannel() {
  ...
  qobject_cast<QVBoxLayout*>(ui->channels->layout())
      ->addWidget(channels.last());
